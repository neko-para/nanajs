...
vector_pattern -> multi_vector_pattern_expand_d {% ([front]) => ({type: 'pattern', ptype: 'vector', front, middle: null, back: null}) %}
    | multi_vector_pattern_expand_d "+" pattern {% ([front, , middle]) => ({type: 'pattern', ptype: 'vector', front, middle, back: null}) %}
    | pattern "+" multi_vector_pattern_expand_d {% ([middle, , back]) => ({type: 'pattern', ptype: 'vector', front, middle, back}) %}
    | multi_vector_pattern_expand_d "+" pattern "+" multi_vector_pattern_expand_d {% ([front, , middle, , back]) => ({type: 'pattern', ptype: 'vector', front, middle, back}) %}
...
set_pattern -> multi_set_pattern_expand_d {% ([front]) => ({type: 'pattern', ptype: 'set', front, middle: null, back: null}) %}
    | multi_set_pattern_expand_d "+" pattern {% ([front, , middle]) => ({type: 'pattern', ptype: 'set', front, middle, back: null}) %}
    | pattern "+" multi_set_pattern_expand_d {% ([middle, , back]) => ({type: 'pattern', ptype: 'set', front, middle, back}) %}
    | multi_set_pattern_expand_d "+" pattern "+" multi_set_pattern_expand_d {% ([front, , middle, , back]) => ({type: 'pattern', ptype: 'set', front, middle, back}) %}

map_pattern_expand -> "{" multi_pattern_d ":" multi_pattern_d "}" {% ([, key, , value]) => ({key, value}) %}
...
map_pattern -> multi_map_pattern_expand_d {% ([front]) => ({type: 'pattern', ptype: 'map', front, middle: null, back: null}) %}
    | multi_map_pattern_expand_d "+" pattern {% ([front, , middle]) => ({type: 'pattern', ptype: 'map', front, middle, back: null}) %}
    | pattern "+" multi_map_pattern_expand_d {% ([middle, , back]) => ({type: 'pattern', ptype: 'map', front, middle, back}) %}
    | multi_map_pattern_expand_d "+" pattern "+" multi_map_pattern_expand_d {% ([front, , middle, , back]) => ({type: 'pattern', ptype: 'map', front, middle, back}) %}

binder_statement -> pattern binder_equ expr ";" {% ([pattern, expose, value]) => ({type: 'bind', pattern, expose, value}) %}
    | symbol multi_symbol_s binder_equ expr ";" {% ([target, gate, expose, value]) => ({type: 'bind', target, expose, value: {type: 'gate', gate, value}}) %}
